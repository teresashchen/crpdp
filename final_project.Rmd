...
normalize %>%
  ...
  nest() %>%
  mutate(phaseplot = map(data, function(data){
    
                data %>% ggplot(aes(x = norm_angle, y = norm_velocity)) +
                              geom_point()
...
test <- normalize %>% 
  filter(id == 1, trial == 1, joint == "ankle")

head(test)

test %>% ggplot(aes(norm_angle, norm_velocity)) +
  geom_point() +
  geom_path() +
  geom_point(data = filter(test, frame == 1), color = "red", size = 3) +
